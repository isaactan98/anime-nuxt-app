  ...
  <div>
    <transition>
      <lottie-player v-if="dragging" class=" grid place-content-center items-center w-full absolute"
                     src="https://lottie.host/ce281adb-dd49-45e6-b756-87b6aaf0814f/KxbGN4rpqQ.json"
                     background="##FFFFFF" speed="1"
                     loop autoplay direction="1" style="height: 100px;" :style="{ top: topPosition + 'px' }"
                     mode="normal">
      </lottie-player>
    </transition>
    ...
    <div class="container px-4 mx-auto min-h-screen relative" :style="{ top: topPosition + 'px' }">
      ...
      <div class="flex overflow-x-auto w-full gap-3 py-4">
        <div v-for="g, k in Genres" :key="k" class="">
          <div
              class="text-xs px-4 py-2 rounded-full bg-purple-500 border-purple-500 text-white whitespace-nowrap cursor-pointer"
              v-if="k == 'all'">
            {{ g }}
          </div>
      ...
      <div class="my-5">
        <div class="flex justify-between text-white">
          ...
          <button class="text-xs flex items-center" @click="navTo('/recent-release?page=1')">View More
            <svg
